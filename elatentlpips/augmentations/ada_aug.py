...
import torch
from . import misc
from .style_ops import upfirdn2d
from .style_ops import grid_sample_gradfix
from .style_ops import conv2d_gradfix
...
class AdaAugment(torch.nn.Module):
    ...
    def forward(self, images, images2, debug_percentile=None):
        ...
        if self.imgfilter > 0:
            ...
            images = images.reshape([batch_size, num_channels, height, width])
            images2 = images2.reshape([1, batch_size * num_channels, height, width])
            images2 = torch.nn.functional.pad(input=images2, pad=[p,p,p,p], mode='reflect')
            images2 = conv2d_gradfix.conv2d(input=images2, weight=Hz_prime.unsqueeze(2), groups=batch_size*num_channels)
            images2 = conv2d_gradfix.conv2d(input=images2, weight=Hz_prime.unsqueeze(3), groups=batch_size*num_channels)
            images2 = images2.reshape([batch_size, num_channels, height, width])
        ...
        if self.noise > 0:
            ...
            to_add = torch.randn([batch_size, num_channels, height, width], device=device) * sigma
            images = images + to_add
            images2 = images2 + to_add
